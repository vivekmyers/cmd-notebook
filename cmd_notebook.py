...
def plot_demo():
    A = all_data[repeats * 45]
    B = all_data[repeats * 85]
    tau = jnp.concatenate([A] + [B])
    lab = [False for x in A] + [True for x in B]
    idx = np.arange(len(tau))
    tau = np.array(list(reversed(tau)))
    lab = np.array(list(reversed(lab)))
    last = jnp.array([0, 20])
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    plt.scatter(*tau.T, c=plt.cm.Blues(np.linspace(0, 1, len(tau))))
    plt.xlim([-30, 30])
    plt.ylim([-30, 30])
    plt.subplot(1, 2, 2)

    vals = jnp.array([dsd(x, last, params["cmd2_metric"]) for x in tau])
    plt.plot(
        idx[lab], [dsd(x, last, params["cmd2_metric"]) for x in tau[lab]], label="cmd2"
    )
    plt.plot(
        idx[~lab],
        [dsd(x, last, params["cmd2_metric"]) for x in tau[~lab]],
        label="cmd2",
    )

    plt.plot(
        idx[lab], [dsd(x, last, params["cmd1_metric"]) for x in tau[lab]], label="cmd1"
    )
    plt.plot(
        idx[~lab],
        [dsd(x, last, params["cmd1_metric"]) for x in tau[~lab]],
        label="cmd1",
    )

    plt.plot(
        idx, [p_goal(x, last, params) - p_goal(x, x, params) for x in tau], label="crl"
    )
    near_origin = jnp.linalg.norm(tau, axis=-1) < 1
    for i in idx[near_origin]:
        plt.plot([i, i], [-5, 10], c="brown", alpha=0.5, linestyle="--")
    # plt.ylim((0, vals.max() * 4.5))
    plt.savefig("demo.png", dpi=250)

#|%%--%%| <WmUz0T11nQ|SySqeIx759>
...
def plot_training():
    ...
    for index, k in enumerate(m.keys()):
        cols = 6
        plt.subplot(len(m) // cols + 1, cols, index + 1)
        v = jnp.nan_to_num(np.array(m[k]))
        if len(v.shape) == 2:
            for col in v.T:
                plt.plot(col)
            ...
            plt.plot(times, v, alpha=0.1, c=c_vec[0])
            m_smooth = gaussian_filter1d(v, 1)
            plt.plot(times, m_smooth, c=c_vec[0])
            # window = m_smooth[max(-t // 2 - 200, -t) // 10 :]
            # mi, ma = np.min(window), np.max(window)
            # d = 0.3 * (ma - mi) + 0.01
            # plt.ylim([mi - d, ma + d])
            plt.title("%s: %.2f" % (k, np.mean(v[-5:])))
    plt.tight_layout()
    plt.savefig("training_curves.png", dpi=150)


# |%%--%%| <SySqeIx759|NcCIJvSM88>
...
for itr in tqdm.trange(int(500)):
    ...
    with Atomic():
        ...
        for k in ["cmd2", "cmd1", "crl", "td", "qrl", "bc"]:
            ...
            m[k + "_success"].append(succ)
            m[k + "_return"].append(ret)
        times.append(t)

        if itr % 5 == 0 or itr < 10:
            plt.clf()
            plot_training()
            plt.show()

# |%%--%%| <NcCIJvSM88|rP3aqkxIcB>


# show demo value plot
plot_demo()

# |%%--%%| <rP3aqkxIcB|csoMCLYQET>
...
# |%%--%%| <csoMCLYQET|UN5oAwtXGV>


pplot("success")
pplot("return")

# |%%--%%| <UN5oAwtXGV|GLPfuwO9Jg>
...
def plot_plan(key, s, g, d, policy, legend=False, ax=None):
    if ax is None:
        ax = plt.gca()
    s = jnp.array(s)
    g = jnp.array(g)
    ...
    masses = waypoint_mass(s, g, ds_sampled, d) / repr_dim
    ...
    trunc = 5000
    ax.scatter(ds_sampled[:trunc, 0], ds_sampled[:trunc, 1], s=3.0, c=masses[:trunc], cmap="viridis_r")
    ssz = 10**2
    gsz = 16**2
    lw = 1.5
    ax.scatter(
...
#|%%--%%| <GLPfuwO9Jg|gGQ4RB3kmx>

key = jax.random.PRNGKey(80)

# |%%--%%| <gGQ4RB3kmx|SgEIgKqzrN>
...
with plt.rc_context(rcParams):
    ...
    for i in tqdm.trange(6):
        ...
        # plot_plan(rng, *coord(), d=metric, ax=axs[i], policy=pol, legend=i==0)
        while not plot_plan(rng, *coord(), d=metric, ax=axs[i], policy=pol, legend=i==0):
            axs[i].clear()
            key, rng = jax.random.split(key)
        axs[i].set_aspect("equal")

    cbar = fig.colorbar(
        axs[0].collections[0], ax=axs[-1], cax=fig.add_axes([0.80, 0.15, 0.02, 0.7])
    )
    cbar.set_label("goal distance")
    plt.subplots_adjust(right=0.88, wspace=-0.5, hspace=0.2)
